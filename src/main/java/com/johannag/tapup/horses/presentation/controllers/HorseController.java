...
import com.johannag.tapup.horses.presentation.schemas.PageHorseResponseDTO;
...
@Tag(name = "Horses", description = "Operations related to horses")
public class HorseController {
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Horse found successfully"),
            @ApiResponse(responseCode = "400", description = "Invalid request", content = {
                    @Content(mediaType = "application/json", schema =
                    @Schema(implementation = ErrorResponse.class))}),
            @ApiResponse(responseCode = "401", description = "Unauthorized: Invalid credentials", content = {
    ...
    public ResponseEntity<HorseResponseDTO> findByUuid(@PathVariable UUID horseUuid) throws HorseNotFoundException {
        ...
        HorseModel horses = horseService.findOneByUuid(horseUuid);
